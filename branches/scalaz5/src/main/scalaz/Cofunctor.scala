...
object Cofunctor {
  ...
  implicit lazy val EqualCofunctor: Cofunctor[Equal] = new Cofunctor[Equal] {
    def comap[A, B](r: Equal[A], f: B => A) = equal[B]((b1, b2) => r equal (f(b1), f(b2)))
  }

  implicit lazy val OrderCofunctor: Cofunctor[Order] = new Cofunctor[Order] {
    def comap[A, B](r: Order[A], f: B => A) = order[B]((b1, b2) => r order (f(b1), f(b2)))
  }

  implicit lazy val ShowCofunctor: Cofunctor[Show] = new Cofunctor[Show] {
  ...
  implicit lazy val MetricSpaceCofunctor: Cofunctor[MetricSpace] = new Cofunctor[MetricSpace] {
    def comap[A, B](r: MetricSpace[A], f: B => A) = metricSpace[B]((b1, b2) => r distance (f(b1), f(b2)))
  }

  import concurrent.{Actor, Effect}

  implicit lazy val ActorCofunctor: Cofunctor[Actor] = new Cofunctor[Actor] {
    def comap[A, B](r: Actor[A], f: B => A): Actor[B] = actor[B](r.onError, (b: B) => (r ! f(b))())(r.strategy)
  }

  implicit lazy val EffectCofunctor: Cofunctor[Effect] = new Cofunctor[Effect] {
