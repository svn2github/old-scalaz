...
trait MAs {
  ...
  // These implement Function1[Int, A], so we need to bump the implicit priority.
...
//  implicit def GenericArrayMA[A](l: GenericArray[A]): MA[GenericArray, A] = ma[GenericArray, A](l)
  // todo replace the last three defs with this, and figure out the compiler error:
  implicit def SeqMA[M[_] <: Seq[_], A](l: M[A]): MA[M, A] = ma[M, A](l) // error B in trait MAs cannot be instantiated from object scalaz.Scalaz
}
