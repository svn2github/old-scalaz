...
object RequestHeader {
   ...
   */
  implicit def entity(eh: EntityHeader): RequestHeader = request.Entity(eh)
  
  /**
   * Converts the given general header into a request header.
   */
  implicit def general(gh: GeneralHeader): RequestHeader = request.General(gh)
  ...
  implicit def StringRequestHeader(s: String): Option[RequestHeader] =
    ...
    (s: Option[GeneralHeader]) |> (request.General(_)) orElse
    (s: Option[EntityHeader]) |> (request.Entity(_))
