...
abstract class BaseApp extends StreamStreamServletApplication {
  ...
  val application = new ServletApplication[Stream, Stream] {
    def application(implicit servlet: HttpServlet, servletRequest: HttpServletRequest, request: Request[Stream]) = {
      val time = new Date()
      // todo pass request and servlet request implicitly. I pass them explicitly here to get a type error (invalid!)
      // from scalac rather than 'no implicit parameters found'.
      val response = route(request, servletRequest) getOrElse resource(x => OK << Stream.fromIterator(x), NotFound.xhtml)(servlet, request)
