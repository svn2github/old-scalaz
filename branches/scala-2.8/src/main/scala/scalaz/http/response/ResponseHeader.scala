...
object ResponseHeader {
  ...
  object General {
     ...
     */
    def unapply(h: ResponseHeader) = h match {
      case response.General(x) => Some(x)
      case _ => None
    }
  }
   ...
   */
  implicit def entity(eh: EntityHeader): ResponseHeader = response.Entity(eh)

  /**
   * Converts the given general header into a response header.
   */
  implicit def general(gh: GeneralHeader): ResponseHeader = response.General(gh)
  ...
  implicit def StringResponseHeader(s: String): Option[ResponseHeader] =
    ...
    (s: Option[GeneralHeader]) |> (response.General(_)) orElse
    (s: Option[EntityHeader]) |> (response.Entity(_))
}
