...
object HttpServletRequest {
  ...
  object MethodPath {
    def unapply(r: Request[IN] forSome { type IN[_]})(implicit hsr: HttpServletRequest): Option[(Method, String)] =
      request.Request.MethodPath.unapply(r) map (ms => (ms._1, ms._2.drop((hsr.getContextPath + "/").length)))
  }

  object Path {
    def unapply(r: Request[IN] forSome { type IN[_]})(implicit hsr: HttpServletRequest): Option[String] =
