...
object Equal {
   ...
   */
  implicit def EqualNonEmptyList[A](implicit e: Equal[A]): Equal[NonEmptyList[A]] = EqualList(e) < ((a: NonEmptyList[A]) => a.toList)
   ...
   */
  implicit def EqualCofunctorW[A](e: Equal[A]): CofunctorW[Equal, A] = cofunctor[Equal, A](e)
}
