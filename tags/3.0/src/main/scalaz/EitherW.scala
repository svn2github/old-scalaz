...
object EitherW {
  ...
  sealed abstract class LeftProjectionW[+A, +B] {
    ...
    /**
     * Maps the given function across this projection.
     */
    def >[X](f: A => X) = p.e.left map f

    /**
     * Binds the given function across this projection.
     */
    def >>=[BB >: B, X](f: A => Either[X, BB]) = p.e.left flatMap f

    /**
     * Anonymous bind the given value across this projection.
     */
    def >>[BB >: B, X](e: Either[X, BB]) = >>=(a => e)
  }
  ...
  object LeftProjectionW {
     ...
     * Wraps a <code>scala.Either.LeftProjection</code>.
     */
    implicit def LeftProjectionLeftProjectionW2[A, B](pp: Either.LeftProjection[A, B]): LeftProjectionW[A, B] = new LeftProjectionW[A, B] {
      val p = pp.e.left
    }
  ...
  sealed abstract class RightProjectionW[+A, +B] {
     ...
     * Maps the given function across this projection.
     */
    def >[X](f: B => X) = p.e.right map f

    /**
     * Binds the given function across this projection.
     */
    def >>=[AA >: A, Y](f: B => Either[AA, Y]) = p.e.right flatMap f

    /**
     * Anonymous bind the given value across this projection.
     */
    def >>[AA >: A, Y](e: Either[AA, Y]) = >>=(b => e)
  }
